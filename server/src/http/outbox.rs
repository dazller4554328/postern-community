...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/outbox/:id", get(get_one).delete(cancel_one))
...
/// scheduled draft.
async fn get_one(State(s): State<AppState>, Path(id): Path<i64>) -> Result<Json<OutboxEntry>> {
    s.vault.require_unlocked()?;
...
async fn reschedule(
    ...
    if !updated {
        return Err(Error::Conflict("outbox entry is no longer pending".into()));
    }
