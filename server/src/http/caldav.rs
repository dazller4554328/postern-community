...
use crate::{
    ...
    error::{Error, Result},
    storage::{CalAccount, CalCalendar, CalEvent, NewCalAccount, NewLocalEvent, PatchLocalEvent},
};

pub fn routes() -> Router<AppState> {
    Router::new()
        .route("/cal/accounts", get(list_accounts).post(create_account))
        .route("/cal/accounts/:id", axum::routing::delete(delete_account))
        ...
        .route(
            "/cal/events/:id",
            get(get_event)
                .patch(update_local_event)
                .delete(delete_local_event),
        )
}
...
async fn trigger_sync(State(s): State<AppState>, Path(id): Path<i64>) -> Result<Json<SyncReport>> {
    s.vault.require_unlocked()?;
...
async fn list_events_in_range(
    ...
    s.vault.require_unlocked()?;
    if q.to < q.from {
        return Err(crate::error::Error::BadRequest("from must be ≤ to".into()));
    }
    ...
    for ev in rows {
        let duration = ev.dtend_utc.map_or(0, |e| (e - ev.dtstart_utc).max(0));
...
async fn create_local_event(
    ...
    s.vault.require_unlocked()?;
    let calendar_id = if let Some(id) = b.calendar_id {
...
async fn delete_local_event(
    ...
    Ok(Json(
        serde_json::json!({ "deleted": id, "removed": removed }),
    ))
}
