...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/accounts/:id/color", post(set_color))
        .route("/accounts/:id/display-name", post(set_display_name))
...
async fn update_credentials(
    ...
    let _ =
        s.db.log_event("account_credentials_updated", Some(&account.email), None);
...
/// Edit the IMAP/SMTP connection details for an existing mailbox (host
/// or provider changed, port moved, etc.). The new IMAP settings are
/// tested against the stored password before anything is saved — same
/// guard as the password-change flow — so a typo can't silently break
/// sync. `kind` (Gmail vs plain IMAP) is re-derived from the new host.
async fn update_server(
    State(s): State<AppState>,
    Path(id): Path<i64>,
    Json(b): Json<ServerBody>,
) -> Result<Json<AccountView>> {
    s.vault.require_unlocked()?;
    let imap_host = b.imap_host.trim().to_string();
    if imap_host.is_empty() {
        return Err(Error::BadRequest("IMAP host is required".into()));
    }
    if b.imap_port == 0 {
        return Err(Error::BadRequest(
            "IMAP port must be between 1 and 65535".into(),
        ));
    }

    let account = s.db.get_account(id)?;
    // Probe the new IMAP host with the existing password before we
    // commit. spawn_blocking-backed, identical to update_credentials.
    let password = s.db.account_password(id, &s.vault)?;
    probe_imap_credentials(
        &s,
        imap_host.clone(),
        b.imap_port,
    ...
    // Clearing the SMTP host clears its port too — sending just won't
    // be configured until it's set again.
    let (smtp_host, smtp_port): (Option<&str>, Option<u16>) = match b
        .smtp_host
        .as_deref()
        .map(str::trim)
        .filter(|h| !h.is_empty())
    {
        Some(h) => (Some(h), b.smtp_port),
        None => (None, None),
    };

    s.db.update_account_server(id, &imap_host, b.imap_port, smtp_host, smtp_port)?;
    let _ = s.db.log_event(
        "account_server_updated",
        Some(&format!(
            "{} → {}:{}",
            account.email, imap_host, b.imap_port
        )),
        None,
    );
    s.scheduler.trigger(id).await;
...
async fn set_sync_enabled(
    ...
    Ok(Json(
        serde_json::json!({ "id": id, "sync_enabled": b.enabled }),
    ))
}
...
async fn set_send_enabled(
    ...
    Ok(Json(
        serde_json::json!({ "id": id, "send_enabled": b.enabled }),
    ))
}
...
async fn set_sync_interval(
    ...
    let _ =
        s.db.log_event("sync_interval_changed", Some(&format!("{actual}s")), None);
...
async fn auto_archive_preview(
    ...
    let acct = s.db.get_account(id)?;
    let age = i64::from(acct.auto_archive_age_days.max(1));
...
async fn retention_preview(
    ...
    let acct = s.db.get_account(id)?;
    let days = i64::from(acct.retention_days.max(1));
...
#[derive(Deserialize)]
struct ColorBody {
    /// Hex string `#rrggbb` (e.g. `#3b82f6`). Pass `null` or empty
    /// string to clear and fall back to the client-side default.
    color: Option<String>,
}
...
async fn set_color(
    ...
    // Reject anything that isn't a 6-digit hex literal. Keeps a stray
    // "blue" or `<script>` from sneaking into a CSS attribute later.
    let color = match b.color.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        None => None,
        Some(c) if is_valid_hex_color(c) => Some(c.to_lowercase()),
        Some(_) => {
            return Err(Error::BadRequest(
                "color must be a 6-digit hex string like #3b82f6".into(),
            ));
        }
    };
    s.db.set_account_color(id, color.as_deref())?;
    let acct = s.db.get_account(id)?;
    Ok(Json(serde_json::json!({
        "id": id,
        "color": acct.color,
...
struct DisplayNameBody {
    /// Sender display name (the "From" name). Pass `null` or empty to
    /// clear it and fall back to the bare email address.
...
async fn set_display_name(
    State(s): State<AppState>,
    Path(id): Path<i64>,
    Json(b): Json<DisplayNameBody>,
) -> Result<Json<serde_json::Value>> {
    let name = match b.display_name.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        None => None,
        Some(n) if n.chars().count() <= 200 => Some(n),
        Some(_) => {
            return Err(Error::BadRequest(
                "display name must be 200 characters or fewer".into(),
            ));
        }
    };
    s.db.set_display_name(id, name)?;
    let acct = s.db.get_account(id)?;
    Ok(Json(serde_json::json!({
        "id": id,
...
async fn start_purge_server_copies(
    ...
    s.vault.require_unlocked()?;
    let account = s.db.get_account(id)?;
    let precheck = body.is_some_and(|Json(b)| b.precheck_only);
