        ...
        Ok(()) => {
            ...
            if totp_status.enabled && !already_trusted {
                ...
                if !pass {
                    ...
                    return Err(Error::BadRequest("wrong or missing two-factor code".into()));
                }
            ...
            let token = new_token();
            s.vault
                .session_open(crate::storage::hash_session_token(&token), ip.clone());
...
async fn rehydrate_ai_on_unlock(s: &AppState) -> crate::error::Result<()> {
    ...
    let chat =
        crate::llm::build_chat_provider(&settings, api_key.as_deref(), bind_iface.as_deref())?;
...
async fn totp_init(State(s): State<AppState>) -> Result<Json<TotpInitReply>> {
    ...
    let secret_b32 = match secret {
        ...
        totp_rs::Secret::Raw(_) => {
            return Err(Error::Other(anyhow::anyhow!("totp secret encoding failed")));
        }
    };
    let totp =
        build_totp(&secret_b32).map_err(|e| Error::Other(anyhow::anyhow!("totp setup: {e}")))?;
...
async fn totp_confirm(
    ...
    s.vault.require_unlocked()?;
    let secret =
        s.db.read_auth_totp_secret(&s.vault)?
...
async fn totp_disable(
    ...
    let pass = match (&b.code, &b.recovery_code) {
        ...
        (Some(c), _) if !c.trim().is_empty() => match s.db.read_auth_totp_secret(&s.vault) {
    ...
    if !pass {
        return Err(Error::BadRequest(
            "wrong code — current 2FA code or a recovery code is required to disable".into(),
        ));
    }
