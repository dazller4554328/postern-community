...
async fn send(
    ...
    // Record sender engagement for the Curated view. Every recipient
    // (To/Cc/Bcc) gets +1 — the user reaching out is the strongest
    // signal that mail FROM these addresses is worth surfacing. Run
    // at enqueue time (not after dispatch) so even cancelled
    // outbox entries count: the user's intent is what matters.
    // Failures are logged, never raised — engagement is best-effort
    // and a DB hiccup here mustn't break the send flow.
    let mut recipients: Vec<String> = req.inner.to.clone();
    recipients.extend(req.inner.cc.iter().cloned());
    recipients.extend(req.inner.bcc.iter().cloned());
    if let Err(e) = s.db.record_engagement_send(&recipients) {
        tracing::warn!(error = %e, "curated: record_engagement_send failed");
    }
