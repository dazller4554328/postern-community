...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/messages/:id/attachment/:index/render-pdf",
            get(render_pdf),
        )
...
async fn body_json(
    ...
    let rendered = render_one(&s, id, q.allow_remote())?;
    Ok(Json(rendered))
}
...
async fn body_html(
    ...
    let doc = format!(
        "<!doctype html>\n\
         ...
         <base target=\"_blank\">\n\
         <style>\n\
           /* Email HTML is authored for a light canvas. Pin one so the \
              UI's dark theme can't render the sender's dark text on a dark \
              background (invisible text); their own bgcolors still win. */\n\
           ...
           /* Keep fixed-width newsletter tables from overflowing the pane. */\n\
           table {{ max-width: 100%; }}\n\
           pre {{ white-space: pre-wrap; word-wrap: break-word; \
             font-family: inherit; }}\n\
           a {{ color: #2a6df4; }}\n\
           blockquote {{ border-left: 3px solid rgba(0,0,0,0.18); \
             padding-left: 0.75rem; margin-left: 0; color: #555; }}\n\
         </style>\n\
         </head><body>\n\
         {body}\n\
         </body></html>",
    ...
    resp.headers_mut().insert(
        ...
        HeaderValue::from_static(
            // img-src is 'self' (not a bare `/img-proxy/` path — that's an
            // invalid CSP source and gets ignored, which silently blocked
            // every proxied image even after the user clicked "Show images").
            // Safe because the sanitizer rewrites every <img src> to either an
            // /img-proxy/ token or the inert placeholder; no other same-origin
    ...
    // SAMEORIGIN (not the global DENY): this document is meant to be
    // framed by the SPA's own message view. CSP frame-ancestors 'self'
    // above is the modern equivalent; both are set for old browsers.
    resp.headers_mut().insert(
...
async fn convert_via_viewer(bytes: &[u8], filename: &str) -> std::result::Result<Vec<u8>, String> {
    ...
    let safe_name: String = filename
        .chars()
        .filter(|c| (c.is_ascii_graphic() || *c == ' ') && !matches!(c, '/' | '\\' | '\0'))
        .take(200)
        .collect();
...
pub(super) fn load_blob(s: &AppState, id: i64) -> Result<Vec<u8>> {
