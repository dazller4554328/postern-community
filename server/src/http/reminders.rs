...
async fn get_reminder(State(s): State<AppState>, Path(id): Path<i64>) -> Result<Json<Reminder>> {
    s.vault.require_unlocked()?;
...
async fn mark_done(State(s): State<AppState>, Path(id): Path<i64>) -> Result<Json<Reminder>> {
    s.vault.require_unlocked()?;
        ...
        SnoozeUntil::Unix(ts) => {
            if ts <= now {
                return Err(Error::BadRequest(
                    "snooze target must be in the future".into(),
                ));
            }
            ts
        }
...
fn tomorrow_9am_utc(now: i64) -> i64 {
    ...
    Utc.with_ymd_and_hms(d.year(), d.month(), d.day(), 9, 0, 0)
        .single()
        .map_or(now + 24 * 3600, |dt| dt.timestamp())
}
