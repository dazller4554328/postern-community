...
struct AccountFolders {
    ...
    avatar_set: String,
    /// User-customised display colour as `#rrggbb`. Drives the
    /// account-coloured pill in the inbox row + drawer chip. NULL =
    /// client picks a deterministic default from `account_id`.
    color: Option<String>,
