...
async fn import_maildir(
    ...
    // Canonicalize before the containment check: a plain `starts_with` on
    // the joined path is fooled by a symlink inside the mount that points
    // out of it. canonicalize() resolves symlinks and `..`, so the check
    // runs on the real target. Both sides are canonicalized so the
    // comparison isn't defeated by the root itself being a symlink.
    let allowed_root = std::fs::canonicalize("/var/lib/postern/import")
        .map_err(|e| Error::BadRequest(format!("import root unavailable: {e}")))?;
    let requested = allowed_root.join(req.path.trim_start_matches('/'));
    let path = std::fs::canonicalize(&requested)
        .map_err(|_| Error::BadRequest("import path does not exist".into()))?;
...
async fn import_archive_zip(
    ...
    let report = tokio::task::spawn_blocking(move || -> Result<ImportReport> {
        ...
        let td = tempfile::tempdir().map_err(|e| Error::Other(anyhow::anyhow!("tempdir: {e}")))?;
...
fn import_mbox_bytes(
    ...
    for msg in split_mbox(raw) {
        ...
        let account_id = if let Some(a) = fixed_account {
            a.id
        } else if let Some(id) = detect_account(msg, &email_map) {
            id
        } else {
            report.skipped += 1;
            continue;
    ...
    for i in 0..zip.len() {
        ...
        if entry.is_dir() {
            std::fs::create_dir_all(&out_path)
                .map_err(|e| Error::Other(anyhow::anyhow!("mkdir {out_path:?}: {e}")))?;
            continue;
        }
        ...
        let mut f = std::fs::File::create(&out_path)
            .map_err(|e| Error::Other(anyhow::anyhow!("create {out_path:?}: {e}")))?;
        ...
        let written = std::io::copy(&mut limited, &mut f)
            .map_err(|e| Error::Other(anyhow::anyhow!("write {out_path:?}: {e}")))?;
        ...
        f.flush()
            .map_err(|e| Error::Other(anyhow::anyhow!("flush {out_path:?}: {e}")))?;
    }
...
fn import_from_dir(
    ...
    for path in &files {
        ...
        let account_id = if let Some(a) = fixed_account {
            a.id
        } else if let Some(id) = detect_account(&body, &email_map) {
            id
        } else {
            report.skipped += 1;
            continue;
