...
async fn sync_inner(
    ...
    let server_url = account
        .server_url
        .as_deref()
        .ok_or_else(|| Error::Other(anyhow::anyhow!("caldav account missing server_url")))?;
    ...
    let (principal, home) = if let (Some(p), Some(h)) = (
        account.principal_url.clone(),
        account.calendar_home_url.clone(),
    ) {
        (p, h)
    } else {
    ...
    for info in collections {
        ...
        for chunk in all_hrefs.chunks(MULTIGET_BATCH) {
            let body = client
                .report(&info.url, "1", &calendar_multiget_body(chunk))
                .await?;
            let responses = parse_multistatus(&body)?;
            for r in responses {
