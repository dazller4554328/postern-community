            ...
            .map_err(|e| Error::Other(anyhow!("{e}")))?
        {
                ...
                match local.as_str() {
                    ...
                    "comp" => {
                        if let Some(r) = current.as_mut() {
                            if stack
                                .iter()
                                .any(|s| s == "supported-calendar-component-set")
                            {
                    ...
                    "current-user-principal" => {
                        capturing_href_into = Some("current_user_principal");
                    }
            ...
            Event::CData(c) => {
                ...
                if top == "calendar-data" {
                    r.props.calendar_data = Some(String::from_utf8_lossy(c.as_ref()).into_owned());
                }
            }
            Event::Eof => break,
            _ => {}
