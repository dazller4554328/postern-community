...
pub async fn discover(client: &Client, server_url: &str) -> Result<(String, String)> {
...
pub async fn list_calendars(client: &Client, calendar_home_url: &str) -> Result<Vec<CalendarInfo>> {
    let body = client
...
mod tests {
    ...
    fn absolute_leaves_already_absolute_alone() {
        let url = absolute_url("https://example.com/base", "https://other.example/abc").unwrap();
