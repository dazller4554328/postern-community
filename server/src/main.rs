...
use sync::{IdleSupervisor, Scheduler};
use vpn::VpnManager;
...
async fn serve(config: Config) -> anyhow::Result<()> {
    ...
    if let Err(e) = backup::restore::consume_pending_restore(&config.data_dir) {
    ...
    // IMAP IDLE supervisor — push-based mailbox change notifications.
    // Runs alongside the polling scheduler: IDLE wakes a sync the
    // moment a new message lands, polling covers servers without
    // IDLE support and folders other than INBOX.
    let idle_supervisor = IdleSupervisor::new(
        db.clone(),
        vpn.clone(),
        vault.clone(),
        scheduler.trigger_sender(),
    );
    let _idle_reconciler = idle_supervisor.spawn_reconciler(std::time::Duration::from_secs(60));

    // Drain the outbox: undo-send windows, scheduled sends, and any
    // 'sending' rows stranded by a previous process restart.
    let _outbox_handle = outbox_worker::spawn(db.clone(), vpn.clone(), vault.clone());

    // AI provider holder — chat provider lives behind an async lock
    // so Settings → AI can swap it at runtime. Seeded from the
    // persisted settings row + the encrypted API key (if any). All
    // best-effort: a misconfigured provider boots an empty holder,
    // AI surfaces grey out in the UI, and the user fixes it from
    ...
    let _backup_scheduler_handle = backup::scheduler::spawn(
        db.clone(),
        vault.clone(),
        state.backup_jobs.clone(),
        config.data_dir.clone(),
        std::env::var("POSTERN_BACKUP_DIR").map_or_else(
            |_| std::path::PathBuf::from("/var/lib/postern/backups"),
...
/// Boot-time AI seeding. Builds the chat provider from the
