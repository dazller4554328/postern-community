...
// `style` is allowed broadly: it's the backbone of HTML-email layout and is
// safe here because (a) the rewriter strips every `url(...)` token from it,
// killing the CSS remote-fetch / tracking vector, and (b) the body renders in
// a fully locked `<iframe sandbox>` so no declaration can script or escape.
// Presentational attributes (`bgcolor`, `width`, `valign`, …) are inert and
// just let newsletters lay out the way their authors intended.
const TAG_ATTRS: &[(&str, &[&str])] = &[
    ...
    (
        "td",
        &[
            "colspan", "rowspan", "align", "valign", "width", "height", "bgcolor", "style",
        ],
    ),
    (
        "th",
        &[
            "colspan", "rowspan", "align", "scope", "valign", "width", "height", "bgcolor", "style",
        ],
    ),
...
fn rewrite_attr(
    ...
    // Inline `style` survives on a few formatting tags (span/div/p), but a
    // `background-image: url(...)` (or any `url()` in the declaration) is a
    // remote fetch that bypasses the `img src` rewrite entirely — a classic
    // CSS tracking-pixel vector. Strip every `url(...)` token so the floor
    // holds regardless of allow_remote. Legit formatting (colour, font,
    // spacing) is preserved.
    if attribute == "style" {
        return Some(Cow::Owned(strip_css_urls(value)));
    }
...
/// Remove every CSS `url(...)` token from an inline `style` value. Used
/// to neutralise remote references (`background-image`, `background`,
/// `list-style-image`, `image-set(url(...))`, etc.) that would otherwise
/// fetch from a sender-controlled host and leak an open signal —
/// independent of the `img src` rewrite and the `allow_remote` gate.
///
/// Matching is case-insensitive on the `url(` opener and tolerates
/// whitespace and quoting inside the parens. The whole `url(...)` is
/// dropped; the surrounding declaration is left intact (e.g.
/// `background: #fff url(x) repeat` → `background: #fff  repeat`), which
/// browsers parse fine — the remote asset simply never loads.
fn strip_css_urls(style: &str) -> String {
    ...
    while i < style.len() {
        if lower[i..].starts_with("url(") {
            // Skip to the matching ')'. CSS doesn't nest url(), so the
            // first ')' closes it; if it's unterminated, drop the rest.
            match style[i + 4..].find(')') {
                Some(rel) => i = i + 4 + rel + 1,
                None => break,
            }
        } else {
            // Advance by one whole char so we never split a multi-byte
            // sequence (style values can carry unicode in `content`). The
            // loop guard guarantees a char here; `break` is defensive so
            // adversarial input can never panic the sanitizer.
            let Some(ch) = style[i..].chars().next() else {
                break;
            };
            out.push(ch);
            i += ch.len_utf8();
...
mod tests {
    ...
    fn plaintext_script_tags_are_neutralised() {
        ...
        let r = render_body(raw, &proxy(), false);
        assert!(
            !r.html.to_lowercase().contains("<script"),
            "got: {}",
            r.html
        );
        assert!(
    ...
    #[test]
    fn strips_css_background_image_url() {
        // A CSS background-image tracking pixel must not survive, even
        // with remote content allowed — it bypasses the img-src path.
        let raw = b"MIME-Version: 1.0\r\nContent-Type: text/html\r\n\r\n\
                    <div style=\"color:#222; background-image: url('https://track.evil/p.gif'); padding:4px\">hi</div>";
        let r = render_body(raw, &proxy(), true);
        assert!(
            !r.html.contains("track.evil"),
            "css url must be stripped: {}",
            r.html
        );
        // Surrounding declarations survive.
        assert!(r.html.contains("color"));
        assert!(r.html.contains("padding"));
    }

    #[test]
    fn strip_css_urls_unit() {
        assert_eq!(
            strip_css_urls("background: #fff url(x.png) repeat"),
            "background: #fff  repeat"
        );
        // Case-insensitive opener, quoted, with whitespace.
        assert_eq!(
            strip_css_urls("background-image: URL( \"https://h/p.gif\" )"),
            "background-image: "
        );
        // No url() — untouched, including unicode.
        assert_eq!(
            strip_css_urls("content: '→'; color: red"),
            "content: '→'; color: red"
        );
        // Unterminated url( — drop the dangling remainder.
        assert_eq!(strip_css_urls("x: url(oops"), "x: ");
    }

    #[test]
    fn keeps_presentational_attrs_but_strips_style_urls_on_table_cells() {
        // Newsletter layout attributes survive so the mail renders as
        // authored, but a `url()` smuggled into a now-allowed `style` on a
        // <td> is still stripped — the widened allowlist must not reopen the
        // CSS tracking vector.
        let raw = b"MIME-Version: 1.0\r\nContent-Type: text/html\r\n\r\n\
                    <table width=\"600\" bgcolor=\"#f5f5f5\"><tr><td valign=\"top\" \
                    style=\"padding:8px; background: url('https://track.evil/p.gif')\">\
                    <font color=\"#333\">hi</font></td></tr></table>";
        let r = render_body(raw, &proxy(), true);
        assert!(r.html.contains("width=\"600\""), "table width kept: {}", r.html);
        assert!(r.html.contains("bgcolor"), "bgcolor kept: {}", r.html);
        assert!(r.html.contains("<font"), "font tag kept: {}", r.html);
        assert!(r.html.contains("padding"), "style padding kept: {}", r.html);
        assert!(
            !r.html.contains("track.evil"),
            "css url in td style must be stripped: {}",
            r.html
        );
    }
