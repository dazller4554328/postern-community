...
impl OllamaProvider {
    /// New provider against `base_url`. The default 5-minute timeout
    /// is generous on purpose — a real RAG question on CPU with a
    /// cold model can run:
    ///   * 10–30s GGUF load (5 GB read + map)
    ///   * 30–60s prompt processing (12 retrieved emails ≈ 4 k tokens
    ///     at 4 CPU cores)
    ///   * 80–130s generation at 3–5 tok/s for ≤ 400 tokens.
    /// Total can exceed two minutes the first time after idle.
    /// Beyond five minutes the model is genuinely hung; aborting is
    /// kinder to the user than letting reqwest hold the socket
    /// indefinitely.
