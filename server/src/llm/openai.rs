...
use super::provider::{ChatRequest, ChatResponse, ChatRole, LlmProvider, PrivacyPosture, Usage};
...
fn reasoning_effort_for(model: &str) -> Option<&'static str> {
    ...
    if m.starts_with("gpt-5") || m.starts_with("gpt5") {
        Some("minimal")
    } else if m.starts_with('o') && m.chars().nth(1).is_some_and(|c| c.is_ascii_digit()) {
        Some("low")
    } else {
        None
    }
...
fn effective_temperature(model: &str, requested: Option<f32>) -> Option<f32> {
    let m = model.to_ascii_lowercase();
    let is_o_series = m.starts_with('o') && m.chars().nth(1).is_some_and(|c| c.is_ascii_digit());
    if is_o_series {
        None
    } else {
        requested
...
impl LlmProvider for OpenAiProvider {
    ...
    async fn transcribe(&self, audio_bytes: Vec<u8>, mime_type: &str) -> Result<String> {
...
mod tests {
    ...
    fn chat_request_serialises_to_openai_shape() {
        ...
        // `stream` field was removed when chat_stream was dropped —
        // we always issue non-streaming requests now.
