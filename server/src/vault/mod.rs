...
const ARGON_M_COST_KIB: u32 = 19_456;
...
fn argon2_derive(password: &str, salt: &[u8]) -> Result<Zeroizing<[u8; 32]>> {
...
fn aead_encrypt(kek: &[u8; 32], plaintext: &[u8]) -> Result<Vec<u8>> {
...
fn aead_decrypt(kek: &[u8; 32], blob: &[u8]) -> Result<Vec<u8>> {
...
fn is_tailnet_cgnat(ip: &str) -> bool {
    let trimmed = ip.trim();
