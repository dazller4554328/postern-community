...
#[allow(dead_code)] // FirstUse payload not read on every branch
...
pub async fn test(
    ...
    let (session, sftp, fingerprint) = open_sftp(config, credential, expected_fingerprint).await?;
    ...
    let _ = session
        .disconnect(russh::Disconnect::ByApplication, "ok", "")
        .await;
...
pub async fn push(
    ...
    sftp.rename(&partial_path, &final_path)
        .await
        .map_err(|e| Error::Other(anyhow::anyhow!("rename {partial_path} → {final_path}: {e}")))?;
...
mod tests {
    ...
    fn join_remote_handles_trailing_slash() {
        assert_eq!(
            join_remote("/home/user/backups", "x.tar.gz"),
            "/home/user/backups/x.tar.gz"
        );
        assert_eq!(
            join_remote("/home/user/backups/", "x.tar.gz"),
            "/home/user/backups/x.tar.gz"
