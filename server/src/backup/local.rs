...
pub fn prune_to_keep(backup_dir: &Path, keep: usize) -> Result<usize> {
    ...
    for old in backups.iter().skip(keep) {
        match delete_backup(backup_dir, &old.filename) {
            ...
            Err(e) => {
                tracing::warn!(filename = %old.filename, error = %e, "retention: prune failed");
            }
        }
    }
    Ok(deleted)
...
fn copy_blobs(src: &Path, dst: &Path) -> Result<usize> {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for entry in entries {
            let Ok(entry) = entry else { continue };
            // Determine the entry type without panicking: if both the
            // dirent type and a metadata fallback fail (e.g. the file was
            // unlinked mid-walk), skip the entry rather than killing the
            // whole backup job.
