...
/// Pull Google's OAuth error out of a token-endpoint error body.
/// Google returns `{"error":"invalid_grant","error_description":"..."}`
/// on 4xx. Returns `(error, error_description)`, each `None` if absent
/// or the body wasn't JSON.
fn parse_google_oauth_error(body: &str) -> (Option<String>, Option<String>) {
...
pub async fn exchange_code(
    ...
    if !http.status().is_success() {
        ...
        let (code, desc) = parse_google_oauth_error(&body);
        return Err(Error::BadRequest(format!(
            "Google rejected the auth code ({status}{}{}). Re-start the connect flow; \
             if it recurs, check the redirect URI and client credentials match Google \
             Cloud Console.",
...
pub async fn refresh_if_expiring(
    ...
    if !http.status().is_success() {
        // Surface Google's OAuth error code so the user (and we) can
        // tell "re-auth needed" apart from "server misconfigured" —
        ...
        let hint = match code.as_deref() {
            Some("invalid_grant") => {
                "The saved Google authorization is no longer valid. \
                The most common cause is the OAuth consent screen still being in \"Testing\" \
                mode, where Google expires refresh tokens after 7 days — publish the app to \
                Production in Google Cloud Console. (Revoking access, or a password change on \
                the Google account, has the same effect.) Then remove and re-add this destination."
            }
            Some("invalid_client") => {
                "Google rejected the OAuth client credentials — check that \
                POSTERN_GDRIVE_CLIENT_ID and POSTERN_GDRIVE_CLIENT_SECRET match the client in \
                Google Cloud Console. Re-adding the destination will not fix this."
            }
            _ => {
                "Remove and re-add the destination; if it recurs, verify the OAuth client \
                config in Google Cloud Console."
            }
        };
        return Err(Error::BadRequest(format!(
            "Google rejected refresh_token ({status}{}{}). {hint}",
            code.map(|c| format!(" {c}")).unwrap_or_default(),
            desc.map(|d| format!(" — {d}")).unwrap_or_default(),
        )));
    }
...
pub async fn ensure_postern_folder(access_token: &str, bind_iface: Option<&str>) -> Result<String> {
    let client = build_client(bind_iface, SHORT_TIMEOUT)?;
    // Search for an existing folder with our name in root.
    let q = format!(
        "name = '{POSTERN_FOLDER_NAME}' and mimeType = 'application/vnd.google-apps.folder' \
         and trashed = false"
    );
...
/// One Drive entry returned by `list_backups`. We carry just what
/// retention pruning needs — the file id (for delete), the name
/// (so callers can sanity-check it's a Postern backup), and the
/// modification time as an RFC 3339 string (so callers can sort
/// without parsing).
...
pub struct DriveBackup {
    ...
    /// Size in bytes, when Drive reports it (it does for our binary
    /// tarballs). Surfaced so the restore UI can show "4.4 GB" before
    /// the user commits to a server-side download.
    pub size: Option<u64>,
}

/// List every (non-trashed) backup tarball Postern has uploaded
/// to the destination folder, newest first. We filter to names
/// matching the `postern-backup-*.tar.gz.age` shape so that other
/// files the user may have stashed in the same folder don't show
/// up in retention logic.
pub async fn list_backups(
    access_token: &str,
    public: &GDrivePublicConfig,
    bind_iface: Option<&str>,
) -> Result<Vec<DriveBackup>> {
    let client = build_client(bind_iface, SHORT_TIMEOUT)?;
    // Drive's `q` syntax supports a `name contains` predicate; we
    // post-filter for the exact prefix in Rust because Drive's
    // contains is case-insensitive substring, not a glob.
    let q = format!(
        "'{}' in parents and trashed = false \
         and name contains 'postern-backup-'",
        public.folder_id
    );
    let resp: serde_json::Value = client
        ...
        .send()
        .await
        .map_err(|e| Error::Other(anyhow::anyhow!("drive list backups: {e}")))?
        .error_for_status()
        .map_err(|e| Error::Other(anyhow::anyhow!("drive list backups: {e}")))?
        .json()
        .await
        .map_err(|e| Error::Other(anyhow::anyhow!("decode drive list backups: {e}")))?;
    ...
    let out = files
        .into_iter()
        .filter_map(|f| {
            let name = f.get("name").and_then(|v| v.as_str())?.to_owned();
            // Belt-and-braces filter — Drive's `contains` is case-
            // insensitive substring, so a file like "old-postern-
            // backup-notes.txt" could slip through. The strict
            // prefix + suffix match keeps retention safe.
            if !name.starts_with("postern-backup-") {
                return None;
            }
            Some(DriveBackup {
                file_id: f.get("id").and_then(|v| v.as_str())?.to_owned(),
                name,
                modified_time: f
                    .get("modifiedTime")
                    .and_then(|v| v.as_str())
...
/// Delete a single file from Drive. Used by retention pruning. The
/// API returns 204 on success; any non-204 with a meaningful body
/// is surfaced as Other so the orchestrator can log and continue.
pub async fn delete_file(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| Error::Other(anyhow::anyhow!("drive delete: {e}")))?;
    if !resp.status().is_success() {
        let status = resp.status();
        let body = resp.text().await.unwrap_or_default();
        return Err(Error::Other(anyhow::anyhow!(
            "drive delete {file_id}: {status} {}",
            body.chars().take(200).collect::<String>()
        )));
    }
    Ok(())
...
/// Download a Drive file by id, streaming it to `dest` on disk.
/// Returns the number of bytes written. Used by cloud-restore: the
/// server pulls the (multi-GB) tarball straight from Drive into the
/// restore staging dir, so the user never has to round-trip it
/// through their browser. Streams chunk-by-chunk — never buffers the
...
pub async fn download_file(
    ...
    // Reuse the wide upload budget — a 4 GB pull over a modest pipe
    // needs far more than the 60s control-plane timeout.
    let client = build_client(bind_iface, UPLOAD_TIMEOUT)?;
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| Error::Other(anyhow::anyhow!("drive download: {e}")))?;
    if !resp.status().is_success() {
        let status = resp.status();
        let body = resp.text().await.unwrap_or_default();
        return Err(Error::BadRequest(format!(
            "Google Drive download failed ({status}): {}",
            body.chars().take(200).collect::<String>()
        )));
    }
    ...
    let mut file = tokio::fs::File::create(dest)
        .await
        .map_err(|e| Error::Other(anyhow::anyhow!("create {dest:?}: {e}")))?;
    let mut stream = resp.bytes_stream();
    let mut total: u64 = 0;
    while let Some(chunk) = stream.next().await {
        let chunk = chunk.map_err(|e| Error::Other(anyhow::anyhow!("download chunk: {e}")))?;
        file.write_all(&chunk)
            .await
            .map_err(|e| Error::Other(anyhow::anyhow!("write chunk: {e}")))?;
        total += chunk.len() as u64;
    }
    file.flush()
        .await
        .map_err(|e| Error::Other(anyhow::anyhow!("flush download: {e}")))?;
    info!(file_id = %file_id, bytes = total, "gdrive: downloaded backup for restore");
    Ok(total)
}
...
mod tests {
    ...
    #[test]
    fn parses_google_oauth_error_body() {
        let (code, desc) = parse_google_oauth_error(
            r#"{"error":"invalid_grant","error_description":"Token has been expired or revoked."}"#,
        );
        assert_eq!(code.as_deref(), Some("invalid_grant"));
        assert_eq!(desc.as_deref(), Some("Token has been expired or revoked."));

        // invalid_client (rotated secret) must be distinguishable.
        let (code, _) = parse_google_oauth_error(r#"{"error":"invalid_client"}"#);
        assert_eq!(code.as_deref(), Some("invalid_client"));

        // Non-JSON / empty bodies degrade gracefully.
        assert_eq!(parse_google_oauth_error("502 Bad Gateway"), (None, None));
        assert_eq!(parse_google_oauth_error(""), (None, None));
    }
