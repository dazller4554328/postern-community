...
pub fn validate_staged_backup(
    ...
    let summary = verify_extracted_backup(&inner, password, staging_id, &tarball)?;
    Ok(summary)
}
...
fn verify_extracted_backup(
    ...
    let accounts: i64 = conn
        ...
        .map_err(|e| {
            ...
            if e.to_string().contains("not a database") {
                Error::BadRequest("wrong master password (DB rejected the derived key)".into())
            } else {
    ...
    let metadata =
        fs::metadata(tarball).map_err(|e| Error::Other(anyhow::anyhow!("stat tarball: {e}")))?;
    ...
    let backup_filename = tarball.file_name().map_or_else(
        || "backup.tar.gz".into(),
...
pub fn consume_pending_restore(data_dir: &Path) -> Result<bool> {
    ...
    if !tarball_path.exists() {
        warn!(
            ?tarball_path,
            "restore marker points at missing tarball — clearing"
        );
...
mod tests {
    ...
    fn locate_backup_root_rejects_empty_tarball() {
        ...
        match err {
            Error::BadRequest(msg) => {
                assert!(msg.contains("not a Postern backup"), "unexpected: {msg}");
            }
    ...
    fn locate_backup_root_finds_single_child() {
        ...
        assert_eq!(found.file_name().unwrap(), "postern-backup-20260101-000000");
    }
    ...
    fn consume_pending_restore_returns_false_when_no_marker() {
        let dir = tempfile::tempdir().unwrap();
        assert!(!consume_pending_restore(dir.path()).unwrap());
    }
