...
pub fn purge_synced_server_copies(
    ...
    let result = match account.kind {
        ...
        AccountKind::Imap => purge_plain_imap(account, password, db, bind_iface, mode, &mut report),
    };
...
fn purge_plain_imap(
    ...
    for folder in folders {
        if let Err(e) = purge_one_imap_folder(
            &mut stream,
            &mut next_tag,
            account.id,
            db,
...
fn parse_list_folders(bytes: &[u8]) -> Vec<String> {
    ...
    for line in text.lines() {
        ...
        // After the flags ')' comes: SP <delimiter> SP <mailbox-name>.
        let rest = &line[open + close + 1..];
        if let Some(name) = parse_mailbox_name(rest) {
            out.push(name);
        }
    }
    out
...
/// Extract the mailbox name from the part of a `* LIST` line that
/// follows the flags list: ` <delimiter> <name>`. The delimiter is a
/// quoted char (`"/"`) or `NIL`; the name is either a quoted string or
/// a bare atom — Cyrus (Fastmail) returns unquoted atoms for simple
/// names, so we must skip the delimiter token rather than just grab the
/// last quoted run (which would yield the delimiter itself). Returns
/// `None` if the structure can't be parsed.
fn parse_mailbox_name(rest: &str) -> Option<String> {
    let after_delim = skip_token(rest)?;
    let name = after_delim.trim();
    if name.is_empty() {
        return None;
    }
    Some(unquote_imap(name))
}

/// Given a string positioned at optional leading whitespace followed by
/// one IMAP token (a quoted string or an atom/`NIL`), return the slice
/// that remains after that token.
fn skip_token(s: &str) -> Option<&str> {
    let s = s.trim_start();
    if let Some(after_open) = s.strip_prefix('"') {
        // Quoted token: find the next unescaped closing quote. Delimiter
        // quotes only ever wrap a single ASCII char, so byte indexing is
        // safe here.
        ...
        // Atom (or `NIL`): up to the next whitespace.
        Some(s.find(char::is_whitespace).map_or("", |p| &s[p..]))
    }
}

/// Strip surrounding IMAP quotes and unescape `\"` / `\\`. Bare atoms
/// pass through unchanged.
fn unquote_imap(s: &str) -> String {
    let s = s.trim();
...
mod tests {
    ...
    #[test]
    fn parse_list_handles_unquoted_atom_names() {
        // Fastmail/Cyrus returns simple mailbox names as bare atoms with
        // a quoted delimiter. The old parser grabbed the last quoted run
        // (the delimiter "/"), collapsing every folder to "/". Regression
        // guard for that.
    ...
    #[test]
    fn parse_list_handles_special_use_flags_with_quoted_names() {
        let raw = b"* LIST (\\HasNoChildren \\Archive) \"/\" \"Archive\"\r\n\
                    a3 OK Completed\r\n";
        let folders = parse_list_folders(raw);
        assert_eq!(folders, vec!["Archive".to_string()]);
    }

    #[test]
    fn parse_list_handles_nil_delimiter() {
        let raw = b"* LIST (\\HasNoChildren) NIL Drafts\r\n\
                    a3 OK Completed\r\n";
        let folders = parse_list_folders(raw);
        assert_eq!(folders, vec!["Drafts".to_string()]);
    }
