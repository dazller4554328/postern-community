...
pub fn send_blocking(
    ...
    let sent_outcome = append_to_sent(&account, &password, &raw, bind_iface.as_deref())
        .unwrap_or_else(|e| {
...
fn hostname_from(email: &str) -> &str {
    email.rsplit_once('@').map_or("localhost", |(_, d)| d)
}

fn parse_mailbox(spec: &str) -> Result<Mailbox> {
    let spec = spec.trim();
    // Fast path: a bare "addr@host" or a well-formed "Name <addr@host>".
    if let Ok(mb) = spec.parse::<Mailbox>() {
        return Ok(mb);
    }
    // Tolerant path: "<display> <addr>" where the display name is
    // unquoted and contains characters RFC 5322 requires quoting — most
    // commonly an email address used as its own display name, e.g.
    // `info@x.com <info@x.com>` (older autocomplete rows whose
    // display_name equals the address). Pull the address out of the
    // angle brackets and rebuild via Mailbox::new, which quotes the
    // display name correctly on serialize. A display name equal to the
    // address (or empty) is dropped so we emit a clean bare address.
    if let (Some(lt), Some(gt)) = (spec.rfind('<'), spec.rfind('>')) {
        if lt < gt {
            let addr = spec[lt + 1..gt].trim();
            ...
            if let Ok(address) = addr.parse::<Address>() {
                ...
                return Ok(Mailbox::new(display, address));
            }
        }
    }
    Err(Error::BadRequest(format!(
        "invalid mailbox {spec}: not a valid email address"
    )))
}
...
pub fn send_read_receipt_blocking(
    ...
    let receipt_to = detail
        .message
        .receipt_to
        .clone()
        .ok_or_else(|| Error::BadRequest("message did not request a read receipt".into()))?;
...
fn build_mdn_message(
    ...
    let _ = writeln!(out, "\r");
    let _ = writeln!(out, "--{boundary}\r");
    let _ = writeln!(out, "Content-Type: message/disposition-notification\r");
    let _ = writeln!(out, "\r");
...
fn append_to_sent(
    ...
    let target = if let Some(name) = resolved {
...
#[cfg(test)]
mod parse_mailbox_tests {
    use super::parse_mailbox;

    #[test]
    fn bare_address() {
        let mb = parse_mailbox("info@nhkinesiology.com").unwrap();
        assert_eq!(mb.email.to_string(), "info@nhkinesiology.com");
        assert!(mb.name.is_none());
    }

    #[test]
    fn address_as_its_own_display_name_collapses() {
        // The exact shape that broke the outbox: unquoted `@` in the
        // display name. Must parse, and drop the redundant name.
        let mb = parse_mailbox("info@nhkinesiology.com <info@nhkinesiology.com>").unwrap();
        assert_eq!(mb.email.to_string(), "info@nhkinesiology.com");
        assert!(
            mb.name.is_none(),
            "redundant display name should be dropped"
        );
    }

    #[test]
    fn real_display_name_kept() {
        let mb = parse_mailbox("Nik H <info@nhkinesiology.com>").unwrap();
        assert_eq!(mb.email.to_string(), "info@nhkinesiology.com");
        assert_eq!(mb.name.as_deref(), Some("Nik H"));
    }
