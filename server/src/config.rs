...
impl Config {
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        let data_dir =
            env::var("POSTERN_DATA_DIR").map_or_else(|_| PathBuf::from("./data"), PathBuf::from);

        let db_path =
            env::var("POSTERN_DB_PATH").map_or_else(|_| data_dir.join("postern.db"), PathBuf::from);

        let blob_dir =
            env::var("POSTERN_BLOB_DIR").map_or_else(|_| data_dir.join("blobs"), PathBuf::from);
