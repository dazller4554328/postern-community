...
fn speak_smtp(
    ...
    // SMTP command injection guard: an address carrying CR/LF (or other
    // control bytes) would split the line and smuggle extra envelope
    // commands. Reject before writing to the wire.
    validate_envelope_addr(from)?;
    write_line(&mut s, &format!("MAIL FROM:<{from}>"))?;
    read_multi(&mut s, 250)?;
    for r in rcpts {
        validate_envelope_addr(r)?;
...
/// Reject envelope addresses that could break the SMTP wire format:
/// control characters (CR/LF/NUL), angle brackets, or whitespace. The
/// address must be non-empty after the surrounding `<>` are added.
fn validate_envelope_addr(addr: &str) -> Result<()> {
    if addr.is_empty()
        || addr
            .bytes()
            .any(|b| b.is_ascii_control() || b == b'<' || b == b'>' || b == b' ')
    {
        return Err(Error::BadRequest(
            "invalid characters in email address".into(),
        ));
    }
    Ok(())
}
...
#[cfg(test)]
mod tests {
    use super::validate_envelope_addr;

    #[test]
    fn accepts_plain_address() {
        assert!(validate_envelope_addr("user@example.com").is_ok());
    }

    #[test]
    fn rejects_crlf_injection() {
        assert!(validate_envelope_addr("a@b.com>\r\nRCPT TO:<victim@evil").is_err());
        assert!(validate_envelope_addr("a@b.com\nDATA").is_err());
        assert!(validate_envelope_addr("a@b\0.com").is_err());
        assert!(validate_envelope_addr("").is_err());
        assert!(validate_envelope_addr("has space@b.com").is_err());
    }
}
