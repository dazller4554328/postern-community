...
impl Db {
    ...
    pub fn ensure_local_cal_account(&self) -> Result<(i64, i64)> {
        ...
        let calendar_id: i64 = if let Some(id) = tx
            .query_row(
    ...
    pub fn upsert_cal_event(&self, calendar_id: i64, ev: &UpsertCalEvent<'_>) -> Result<i64> {
        let conn = self.pool().get()?;
        let now = chrono::Utc::now().timestamp();
        conn.execute(
    ...
    pub fn list_cal_events_in_range(&self, from_utc: i64, to_utc: i64) -> Result<Vec<CalEvent>> {
        let conn = self.pool().get()?;
        let mut stmt = conn.prepare(
    ...
    pub fn cal_event_update_local(&self, id: i64, p: &PatchLocalEvent<'_>) -> Result<()> {
        ...
        let rrule = p.rrule.map(std::borrow::ToOwned::to_owned).or(cur.rrule);
...
fn build_local_ics(uid: &str, e: &NewLocalEvent<'_>) -> String {
    ...
    let _ = writeln!(
        s,
        "DTSTAMP:{}",
        format_ical_utc(chrono::Utc::now().timestamp())
    );
    if e.all_day {
...
fn format_ical_date(ts: i64) -> String {
    chrono::DateTime::<chrono::Utc>::from_timestamp(ts, 0)
        .map_or_else(|| "19700101".into(), |dt| dt.format("%Y%m%d").to_string())
}
