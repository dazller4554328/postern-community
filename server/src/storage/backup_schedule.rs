...
pub fn should_fire_now(sched: &BackupSchedule, now: chrono::DateTime<chrono::Local>) -> bool {
...
mod tests {
    ...
    fn frequency_parse_round_trips() {
        assert_eq!(BackupFrequency::parse("daily").unwrap().as_str(), "daily");
        assert_eq!(BackupFrequency::parse("weekly").unwrap().as_str(), "weekly");
