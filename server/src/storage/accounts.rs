...
impl Db {
    ...
    pub fn set_display_name(&self, id: i64, name: Option<&str>) -> Result<()> {
        let conn = self.pool().get()?;
        let normalized = name.map(str::trim).filter(|s| !s.is_empty());
        let n = conn.execute(
    ...
    pub fn set_archive_folder(&self, id: i64, folder: Option<&str>) -> Result<()> {
        let conn = self.pool().get()?;
        let normalized = folder.map(str::trim).filter(|s| !s.is_empty());
        let n = conn.execute(
    ...
    pub fn set_signature(&self, id: i64, html: Option<&str>, plain: Option<&str>) -> Result<()> {
        let conn = self.pool().get()?;
    ...
    pub fn set_avatar(&self, id: i64, seed: Option<&str>, set: &str) -> Result<()> {
        ...
        let normalized = seed.map(str::trim).filter(|s| !s.is_empty());
        let conn = self.pool().get()?;
        let n = conn.execute(
    ...
    /// Update an account's IMAP/SMTP connection details and re-derive
    /// `kind` from the new IMAP host. The caller probes the new settings
    /// first (see the `/server` handler) so we never persist a host that
    /// doesn't authenticate.
    pub fn update_account_server(
        &self,
        id: i64,
        imap_host: &str,
        imap_port: u16,
        ...
        conn.execute(
            "UPDATE accounts
                SET imap_host = ?1, imap_port = ?2,
                    smtp_host = ?3, smtp_port = ?4,
                    kind = ?5
              WHERE id = ?6",
            params![
                imap_host,
    ...
    pub fn account_password(&self, id: i64, vault: &Vault) -> Result<String> {
        ...
        String::from_utf8(plain)
            .map_err(|e| crate::error::Error::Other(anyhow::anyhow!("app password not utf8: {e}")))
    }
}
