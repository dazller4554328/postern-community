...
impl BlobStore {
    ...
    pub fn get(&self, hex: &str) -> Result<Vec<u8>> {
        ...
        if raw.first() == Some(&BLOB_VERSION) {
            let key = self
                .current_key()
                .ok_or_else(|| Error::Locked("vault locked; cannot decrypt blob".into()))?;
...
mod tests {
    ...
    fn encrypted_roundtrip_under_key() {
        ...
        let on_disk =
            std::fs::read(dir.path().join(&hex[0..2]).join(&hex[2..4]).join(&hex)).unwrap();
    ...
    fn migrate_encrypts_plaintext_idempotently() {
        ...
        let on_disk = std::fs::read(dir.path().join(&h1[0..2]).join(&h1[2..4]).join(&h1)).unwrap();
        assert_eq!(on_disk[0], BLOB_VERSION);
