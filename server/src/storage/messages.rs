...
pub struct MessageDetail {
    ...
    /// Parent Message-ID parsed from the raw blob's `In-Reply-To`
    /// header. Populated by the HTTP layer (storage doesn't have blob
    /// access) and used by compose-reply to emit a standards-compliant
    /// References chain — receiving clients (Gmail, Outlook) still
    /// thread by References even though Postern itself no longer does.
    #[serde(default)]
    pub in_reply_to: Option<String>,
    /// Full `References` list from the raw blob, oldest-first per RFC.
    #[serde(default)]
...
impl Db {
        ...
        let msg_id = if let Some(id) = existing {
            ...
            tx.execute(
                "INSERT INTO messages
                   (account_id, message_id, subject, from_addr, to_addrs, cc_addrs,
                    date_utc, blob_sha256, size_bytes, snippet, body_text,
                    has_attachments, is_read, is_encrypted, receipt_to)
    ...
    pub fn relabel_message(&self, id: i64, account_id: i64, label_names: &[&str]) -> Result<()> {
        ...
        for name in label_names {
            let label_id: Option<i64> = tx
                ...
                .optional()?;
            let lid = if let Some(lid) = label_id {
                lid
            } else {
    ...
    /// Blob hash of the stored copy of a message, if present. Used by
    /// delete_after_sync to prove the local copy is byte-identical to the
    /// server copy before the server copy is purged.
    pub fn message_blob_hash(&self, account_id: i64, message_id: &str) -> Result<Option<String>> {
        let conn = self.pool().get()?;
        let hash = conn
            .query_row(
                "SELECT blob_sha256 FROM messages WHERE account_id = ?1 AND message_id = ?2",
...
#[allow(dead_code)] // candidate DTO; `is_read` populated but not yet read
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn insert_message(db: &Db, account_id: i64, message_id: &str) {
        let conn = db.pool().get().unwrap();
        conn.execute(
            "INSERT INTO messages(account_id, message_id, date_utc,
    ...
    fn messages_present_scopes_to_account() {
        ...
        assert_eq!(
            db.messages_present(account_a, &["<shared@local>".into()])
                .unwrap()
                .len(),
            1
        );
        assert_eq!(
            db.messages_present(account_b, &["<shared@local>".into()])
                .unwrap()
                .len(),
            0
        );
    ...
    fn remove_label_drops_only_named_label() {
        ...
        assert_eq!(removed, 1);
        assert_eq!(
            label_set_for(&db, account_id, "<a@local>"),
    ...
    fn remove_label_scopes_to_account() {
        ...
        db.remove_label_from_messages(account_a, &["<a@local>".into()], "INBOX")
            .unwrap();
        assert_eq!(
            label_set_for(&db, account_a, "<a@local>"),
            Vec::<String>::new()
        );
        assert_eq!(
            label_set_for(&db, account_b, "<a@local>"),
            vec!["INBOX".to_string()]
        );
    }
