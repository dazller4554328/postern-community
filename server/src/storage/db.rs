...
const MIGRATIONS: &[(&str, &str)] = &[
    ...
    ("0036_ai", include_str!("../../migrations/0036_ai.sql")),
    (
    ...
    (
        "0050_notes",
        include_str!("../../migrations/0050_notes.sql"),
    ),
    (
        "0051_calendar_local",
        include_str!("../../migrations/0051_calendar_local.sql"),
    ),
    (
        "0052_account_color",
        include_str!("../../migrations/0052_account_color.sql"),
    ),
    (
        "0053_trusted_senders",
        include_str!("../../migrations/0053_trusted_senders.sql"),
    ),
    (
        "0054_sender_engagement",
        include_str!("../../migrations/0054_sender_engagement.sql"),
    ),
    (
        "0055_contact_photos",
        include_str!("../../migrations/0055_contact_photos.sql"),
    ),
    (
        "0056_drop_sender_engagement",
        include_str!("../../migrations/0056_drop_sender_engagement.sql"),
    ),
    (
        "0057_drop_ai_embeddings",
        include_str!("../../migrations/0057_drop_ai_embeddings.sql"),
    ),
    (
        "0058_drop_ai_history_activity",
        include_str!("../../migrations/0058_drop_ai_history_activity.sql"),
    ),
    (
        "0059_drop_lockdown",
        include_str!("../../migrations/0059_drop_lockdown.sql"),
    ),
    (
        "0060_drop_threading",
        include_str!("../../migrations/0060_drop_threading.sql"),
    ),
    (
        "0061_note_revisions",
        include_str!("../../migrations/0061_note_revisions.sql"),
    ),
    (
        "0062_strip_foreign_gmail_labels_on_imap",
        include_str!("../../migrations/0062_strip_foreign_gmail_labels_on_imap.sql"),
    ),
    (
        "0063_remote_avatars_setting",
        include_str!("../../migrations/0063_remote_avatars_setting.sql"),
    ),
];
...
impl Db {
    ...
    fn build_pool(path: &Path, key_state: KeyStateRef) -> Result<Pool<SqliteConnectionManager>> {
        ...
        let manager = SqliteConnectionManager::file(path).with_init(move |c| {
            ...
            // database. Recover on poison — skipping the PRAGMA would open
            // the connection keyless and fail later with a misleading
    ...
    pub fn rekey_pool(&self, key_hex: String) -> Result<()> {
        // Recover on poison rather than skipping the write: silently
        // leaving the old key in place would make every later query fail
        // with an opaque "file is not a database" while unlock reports
        // success. Same recovery rationale as `pool()`.
        self.key_state
            .write()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
        ...
        *self
            .pool
            .write()
            .unwrap_or_else(std::sync::PoisonError::into_inner) = new_pool;
        Ok(())
    }
    ...
    pub fn pool(&self) -> Pool<SqliteConnectionManager> {
        // Recover the guard on poison rather than cascading a panic: this
        // RwLock protects a clonable pool handle, not an invariant a prior
        // panic could have corrupted. A poisoned pool must not take down
        // every subsequent DB access.
        self.pool
            .read()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone()
    ...
    #[must_use]
    #[allow(dead_code)] // data-dir accessor kept for callers/tests
