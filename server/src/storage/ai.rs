...
impl Db {
    ...
    /// Indexable message count — total minus messages that current
    /// AI exclusions filter out. Mirrors the WHERE clauses used by
    /// `messages_missing_embedding` so the coverage bar stops at
    /// "100% of what we *intend* to index", instead of hanging at
    /// 70% forever because excluded mail counts toward the
    /// denominator.
    pub fn total_indexable_count(
        &self,
        ...
        let sql = format!(
            "SELECT COUNT(*) FROM messages m WHERE 1=1 {sender_clause} {label_clause}"
        );
