...
impl Db {
    ...
    pub fn set_backup_destination_fingerprint(&self, id: i64, fingerprint: &str) -> Result<()> {
        let conn = self.pool().get()?;
        let n = conn.execute(
    ...
    pub fn record_destination_push_ok(&self, id: i64, filename: &str) -> Result<()> {
        let conn = self.pool().get()?;
        conn.execute(
...
fn row_to_destination(r: &rusqlite::Row) -> rusqlite::Result<BackupDestination> {
    ...
    let public_config: serde_json::Value =
        serde_json::from_str(&public_json).unwrap_or_else(|_| {
