...
pub fn parse(input: &str, now_utc: i64) -> ParsedQuery {
    ...
    // Skip when the term is (or ends in) a phrase: FTS5 doesn't
    // accept `*` after a closing quote.
...
/// Clean a bare FTS term: strip characters FTS5 would reject as
/// operators (`*`, `(`, `)`, `:` outside column-scope terms), keep
/// alphanumerics and common punctuation that matters in email
/// addresses / filenames. Returns a string that is safe to drop into
/// an FTS5 MATCH expression — alphanumeric+`_`-only inputs come back
/// as bare words; anything containing `.`, `-`, `@` etc. comes back
/// wrapped in double quotes so FTS5 treats it as a phrase (the
/// default tokenizer splits on those characters internally, so the
/// phrase still matches the original substring).
fn clean_fts_term(s: &str) -> String {
    ...
    if filtered.chars().all(is_fts_bareword_char) {
        filtered
    } else {
        // Phrase form: wrap so FTS5 doesn't choke on @, ., -, etc.
        format!("\"{filtered}\"")
    }
}

/// FTS5 barewords accept ASCII alphanumerics + underscore. Anything
/// outside that set has to live inside a quoted phrase.
fn is_fts_bareword_char(c: char) -> bool {
...
fn apply_operator(
    ...
    match op.to_ascii_lowercase().as_str() {
        ...
        "is" => match unquoted.to_ascii_lowercase().as_str() {
            "unread" => q.is_read = Some(negated),
            "read" => q.is_read = Some(!negated),
...
mod tests {
    ...
    #[test]
    fn email_addresses_become_phrase_terms() {
        // Bare email click: search bar gets "from:alice@example.com" (or
        // the quoted form `from:"alice@example.com"` from the inbox-row
        // shortcut). FTS5 chokes on '@' as a bareword character — we
        // need the email to land inside double quotes.
        let q = parse("from:alice@example.com", NOW);
        assert_eq!(q.fts_expr, "from_addr:\"alice@example.com\"");

        // Same for plain keywords containing '.', '-', '@'.
        let q2 = parse("alice@example.com", NOW);
        assert_eq!(q2.fts_expr, "\"alice@example.com\"");
        // No `*` appended — phrase terms can't carry a wildcard.
        assert!(!q2.fts_expr.ends_with('*'));
    }
}
