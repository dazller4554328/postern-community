...
impl Db {
    ...
    pub fn read_auth_totp_secret(&self, vault: &crate::vault::Vault) -> Result<Option<String>> {
        let conn = self.pool().get()?;
        let secret_ref: Option<String> = conn
            .query_row("SELECT secret_ref FROM auth_totp WHERE id = 1", [], |r| {
                r.get(0)
            })
            .optional()?
            .flatten();
