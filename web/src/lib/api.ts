...
/** One row of the curated view — same as MessageListItem plus the
 *  composite score that ranked it. Score is recency × engagement;
 *  in [0, 1] roughly. Surfaces eventually as a "why" hint in the
 *  UI; for now the frontend just consumes the order. */
export interface CuratedListItem extends MessageListItem {
...
export const api = {
  ...
  /// Curated view — engagement-ranked message list. Same shape as
  /// listMessages, plus a curated_score per row so the UI can show
  /// or sort on it. Backend ranks by sender-engagement count +
  /// recency (Phase 1, no AI).
  listCurated: (
    params: {
      account_id?: number;
      limit?: number;
    ...
    if (params.offset != null) q.set('offset', String(params.offset));
    const qs = q.toString();
    return request<CuratedListItem[]>(`/api/curated/messages${qs ? `?${qs}` : ''}`);
  },
  ...
  // Trusted senders — allowlist that survives "Not spam" actions.
  // Sync auto-rescues any spam-folder mail whose sender is on this list.
  listTrustedSenders: () =>
    request<TrustedSender[]>('/api/trusted-senders'),
  ...
  outboxClearFailures: () =>
    request<{ removed: number }>('/api/outbox/recent-failures', {
      method: 'DELETE'
    }),
  ...
  // ---- Secure notes (markdown body, vault-encrypted at rest) ----
...
export interface AiSettingsTest {
  /// Default 'chat'. Use 'embed' to probe the embedding provider
  /// instead — the embed test additionally generates a one-token
  /// embedding and returns its vector dimension, so it catches
  /// "model isn't pulled into Ollama yet" errors that a bare
  /// reachability probe wouldn't see.
  target?: 'chat' | 'embed';
  provider_kind: AiProviderKind;
  chat_model?: string;
  embed_model?: string;
  base_url?: string | null;
  api_key?: string | null;
  /// Only consulted when target='embed'. Falls back to the chat-side
  /// fields when omitted, so the common "everything is OpenAI" /
  /// "everything is Ollama" config tests cleanly with one form.
  embed_provider_kind?: AiEmbedProviderKind;
  embed_base_url?: string | null;
  embed_api_key?: string | null;
}
...
export interface AiSettingsTestResult {
  ...
  message: string | null;
  /// Embed test only — dimension of the produced vector.
  vector_dim?: number;
  /// Embed test only — round-trip latency in ms.
  embed_ms?: number;
}
