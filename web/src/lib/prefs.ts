...
export interface Prefs {
  ...
  /**
   * Zebra striping in the message list — every second row gets a
   * markedly tinted background. Off by default (the subtle 2 %
   * alternation is already in the default stylesheet); flipping this
   * on bumps the alternating fill to ~10–14 % so long lists are
   * easier to scan.
   */
  zebraRows: boolean;
}
...
export const FONT_STACKS: Record<FontId, string> = {
  ...
  // Humanist sans — friendly, slightly calligraphic shapes, reads
  // well at small sizes. Avenir on macOS / iOS, Source Sans on
  // Linux installs of Adobe fonts, IBM Plex / Nunito as widely-
  // installed third-party fallbacks.
  humanist:
    '"Avenir Next", "Avenir", "Source Sans 3", "Source Sans Pro", "IBM Plex Sans", "Nunito Sans", system-ui, sans-serif',
  // Geometric sans — modernist, more impersonal than humanist.
  // Futura on macOS, Century Gothic on Windows.
  geometric:
    '"Futura", "Avenir Next Condensed", "Century Gothic", "URW Gothic", "Trebuchet MS", system-ui, sans-serif',
  // Neutral grotesque — Helvetica family. The "I want it to look
  // like a printed business document" choice.
  grotesque:
    '"Helvetica Neue", Helvetica, "Liberation Sans", Arial, sans-serif',
  // Slab serif — square serifs, chunky, distinctive for headers.
  // Rockwell on Windows, Roboto Slab on Android, Charter elsewhere.
  slab:
    '"Roboto Slab", "Rockwell", "Rockwell Nova", "Charter", "Bitstream Charter", serif',
  // Book/literary serif — modern transitional faces tuned for
  // continuous reading. Charter is the gold standard; Source
  // Serif and Georgia are the realistic fallbacks.
  book:
    '"Charter", "Bitstream Charter", "Source Serif 4", "Source Serif Pro", Georgia, "Cambria", serif',
  // Compact / condensed — tighter horizontal advance, useful for
  // dense list views. SF Compact on Apple, Inter Tight elsewhere.
  compact:
    '"SF Compact Text", "SF Compact", "Inter Tight", "Roboto Condensed", "Liberation Sans Narrow", system-ui, sans-serif',
  // Dyslexia-friendly — OpenDyslexic if installed (deliberate user
  // choice, common in accessibility configs), else Comic-style
  // fonts which research suggests help some readers (no consensus,
  // but choice belongs to the user).
  dyslexia:
    '"OpenDyslexic", "Comic Neue", "Comic Sans MS", "Lexend", system-ui, sans-serif'
};
...
export const FONT_LABELS: Record<FontId, string> = {
  ...
  mono: 'Monospace',
  humanist: 'Humanist sans — Avenir / Source',
  geometric: 'Geometric sans — Futura',
  grotesque: 'Grotesque — Helvetica',
  slab: 'Slab serif — Rockwell / Charter',
  book: 'Book serif — Charter / Source',
  compact: 'Compact — SF Compact / Inter Tight',
  dyslexia: 'Dyslexia-friendly — OpenDyslexic'
};
  ...
  try {
    ...
    if (raw) {
      ...
      return {
        ...
        zebraRows:
          typeof parsed.zebraRows === 'boolean'
            ? parsed.zebraRows
            : DEFAULT.zebraRows
      };
    }
