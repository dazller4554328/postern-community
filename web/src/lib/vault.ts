...
import { setAuthExpiredHandler } from './api/_client';
...
if (browser) {
  ...
  // Server says the session is gone — flip the gate immediately so
  // the user sees the unlock screen instead of a stale inbox that
  // silently stops fetching. Triggered by any 401 / 423 response
  // from the API; covers vault auto-lock, IP-change auto-lock,
  // session-cookie expiry, and the explicit lock button on another
  // device. Idempotent — repeated calls while already locked are a
  // no-op so a burst of in-flight 401s doesn't ping-pong the store.
  setAuthExpiredHandler(() => {
    if (get(vaultState) !== 'locked') {
