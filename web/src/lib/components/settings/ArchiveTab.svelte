            ...
            <div class="account-panel">
              ...
              <fieldset class="nested" disabled={!d.archive_enabled}>
                ...
                <div class="auto-block retention">
                  <div class="auto-head">
                    <strong>Delete provider-side after N days</strong>
                    <InfoBubble text="Periodic sweep — after each sync, deletes Inbox messages on the mail provider (Gmail → Trash, plain IMAP → EXPUNGE) that are older than the threshold below. Postern keeps its local copy, so nothing disappears from here — this is purely to free provider-side quota. Starred messages are always skipped. Distinct from Settings → Mailboxes → 'On-download deletion' (which deletes on download with no grace period); only one of the two needs to be on at a time." />
                  </div>
                  <label class="toggle-row compact" for="ret-en-{a.id}">
                    ...
                    />
                    <span>Delete provider-side Inbox messages older than the threshold below</span>
                  </label>
