<script lang="ts">
  ...
  async function runTest(target: 'chat' | 'embed' = 'chat') {
    testing = true;
    testResult = null;
    saveError = null;
    try {
      ...
      testResult = {
        ok: false,
        provider: target === 'embed' ? form.embed_provider_kind : form.provider_kind,
  ...
  {:else}
    ...
    <div class="ai-form-grid">
      ...
      <div class="ai-form-card">
        ...
        <!-- Base URL when embed_provider_kind is openai_compat or ollama.
             Previously this was hidden when both chat and embed were Ollama
             (assuming embed would inherit chat's URL). That assumption was
             one-way: if a user had ever saved an embed URL while the field
             was visible, the saved value persisted with no UI to clear it.
             Now always visible for both kinds — leave blank to inherit. -->
        {#if embedNeedsBaseUrl || embedSupportsBaseUrlOverride}
      <div class="field">
        <div class="field-label">
          ...
          <InfoBubble text={embedNeedsBaseUrl
            ...
            : 'Override where the embed Ollama lives. Leave blank to inherit the chat base URL above (recommended when both are the same box).'} />
        </div>
        <input
          id="ai-embed-base-url"
          type="url"
          bind:value={form.embed_base_url}
          placeholder={embedNeedsBaseUrl ? 'http://vllm.lan:8000/v1' : (form.base_url || 'http://localhost:11434')}
          autocomplete="off"
          spellcheck="false"
        />
      </div>
    ...
    <div class="form-footer">
      <div class="actions">
        ...
        <button
          ...
          title="Generates a one-token embedding to verify the model is loaded — catches 'model not pulled' errors that a reachability check would miss."
        >
      ...
      {#if testResult}
        <p class="status-line {testResult.ok ? 'ok' : 'err'}">
          {#if testResult.ok}
            ...
            {#if testResult.vector_dim != null}
              · {testResult.vector_dim}-dim vector
              {#if testResult.embed_ms != null}
                in {testResult.embed_ms} ms
              {/if}
            {/if}
          {:else}
