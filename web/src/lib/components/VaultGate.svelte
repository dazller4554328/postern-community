<script lang="ts">
  ...
  // Same condition gates whether we ask for a 2FA code: a valid
  // persistent cookie skips 2FA on the server, so the field would
  // just be a useless extra prompt. If our local mirror is wrong
  // (cookies cleared, cookie revoked elsewhere) the server returns
  // "missing 2FA code" and the catch-block below clears the mirror
  // so the field reappears on the retry.
  let needTotp = $derived(totpEnabled && !hideRemember);
    ...
    try {
      if ($vaultState === 'uninitialized') {
        ...
        await unlockVault(password, effectiveRemember, {
          totpCode: !useRecovery && needTotp ? totpCode.trim() : undefined,
          recoveryCode: useRecovery && needTotp ? recoveryCode.trim() : undefined
        });
      }
      password = '';
      confirm = '';
      ...
      err = e instanceof Error ? e.message : String(e);
      // Self-heal on a 2FA-related rejection. Two cases collapse here:
      //   (1) totpEnabled was false in our mirror — flip it on so the
      //       field appears on the retry.
      //   (2) we hid the field thinking the device was trusted, but
      //       the server disagreed (cookie revoked elsewhere, cleared
      //       by the browser, etc.). Drop the local "trusted" mirror
      //       and the cookie-bound store so needTotp re-evaluates to
      //       true and the field reappears.
      if (err && /two-factor|2fa/i.test(err)) {
        if (!totpEnabled) {
          totpEnabled = true;
          try {
  ...
  <div class="screen">
    <div class="card">
      ...
      <form onsubmit={onSubmit}>
        ...
        {/if}
        {#if $vaultState !== 'uninitialized' && needTotp}
          {#if !useRecovery}
            <label>
