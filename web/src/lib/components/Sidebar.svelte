...
<aside>
  ...
  <nav>
    ...
    <div class="tools">
      ...
      <a
        class="row tool-row"
        class:active={isToolActive('/curated')}
        href="/curated"
        title="Curated view — mail from people you correspond with, ranked above the noise"
      >
        <span class="tool-icon" aria-hidden="true">
          <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.65" stroke-linecap="round" stroke-linejoin="round">
            <path d="M3.5 5h13M5 9.5h10M7 14h6"/>
            <circle cx="14.5" cy="5" r="1.6" fill="currentColor"/>
          </svg>
        </span>
          ...
          >
            <span class="acct-avatar-wrap">
              <span
                class="acct-color"
                class:collapsed
                style:background-color={sidebarAccountColor(acct)}
                aria-hidden="true"
                title={acct.email}
              ></span>
            </span>
...
<style>
  ...
  .acct-header:hover .acct-color {
    transform: scale(1.08);
  }
  .acct-color.collapsed {
