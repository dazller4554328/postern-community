  ...
  <div class="content" class:split-horizontal={splitOrient === 'horizontal' && (selectedId !== null || selectedThread !== null)} class:split-vertical={splitOrient === 'vertical' && (selectedId !== null || selectedThread !== null)} class:no-preview={selectedId === null && selectedThread === null}>
    <section class="list-pane">
          ...
          {#if activeAccount != null}
            ...
            {#if acct}
              <img
                ...
                src={robohashUrl(acct.avatar_seed?.trim() || acct.email, acct.avatar_set ?? 'set1', 112)}
                alt=""
                title={acct.email}
                loading="lazy"
              />
      ...
      {:else if listMode === 'threads' && !isSearching}
        ...
        {:else}
          <div class="list-scroll">
            <ul class="thread-list density-{density}">
      ...
      {:else}
        ...
        {/if}
        <div class="list-scroll">
          <ul class="msg-list density-{density} style-{rowStyle}" class:bulk-active={checkedCount > 0}>
              ...
              <li class:checked={isChecked(m.id)}>
                ...
                >
                  <span
                    ...
                    aria-label={`${m.is_read ? 'Read' : 'Unread'}${m.is_encrypted ? ' encrypted' : ''}`}
                  >
                    {#if m.is_encrypted}
                      {#if m.is_read}
                        <!-- Read + encrypted: outline envelope with a gold padlock.
                             Keeping the padlock gold in every state makes
                             PGP messages easy to spot whether they've been
                             read or not. -->
                        <svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round" stroke-linecap="round" aria-hidden="true">
                          <path d="M3 9.5 12 15l9-5.5"/>
                          <path d="M3 9.5v10h18v-10"/>
                          <path d="M3 9.5 12 4l9 5.5"/>
                          <rect x="13.5" y="12.5" width="7" height="6" rx="1" fill="#fbbf24" stroke="none"/>
                          <path d="M15 12.5v-1.2a2 2 0 0 1 4 0v1.2" stroke="#fbbf24" stroke-width="1.4"/>
                        </svg>
                      {:else}
                        <!-- Unread + encrypted: solid envelope, still-gold padlock. -->
                        <svg viewBox="0 0 24 24" width="17" height="17" aria-hidden="true">
                          <path fill="currentColor" d="M2.5 6.5A1.5 1.5 0 0 1 4 5h16a1.5 1.5 0 0 1 1.5 1.5v11A1.5 1.5 0 0 1 20 19H4a1.5 1.5 0 0 1-1.5-1.5v-11Zm1.6.2 7.9 5.5 7.9-5.5-.5-.2H4.5l-.4.2Z"/>
                          <rect x="13.5" y="12" width="7.5" height="6.5" rx="1.1" fill="#fbbf24" stroke="#fff" stroke-width="0.8"/>
                          <path d="M15 12v-1.2a2.2 2.2 0 0 1 4.4 0V12" fill="none" stroke="#fbbf24" stroke-width="1.3" stroke-linecap="round"/>
                        </svg>
                      {/if}
                    {:else if m.is_read}
                      <!-- Read + plain: open outline envelope -->
