<script lang="ts">
  ...
  // Notes companion — opens /notes in a chrome-stripped popup so the
  // user can read/copy notes without abandoning their draft. On mobile
  // browsers `window.open` opens a new tab (the features string is
  // ignored), which still preserves the compose draft because the
  // tab stays alive in the background.
  function openNotes() {
...
{#if suggestions.length > 0 && acFocused !== null && acRect}
  <ul
    class="ac-list"
    style:top="{acRect.top}px"
    style:left="{acRect.left}px"
    style:width="{acRect.width}px"
  >
    {#each suggestions as s (s)}
      <li>
        <button type="button" onmousedown={(e) => { e.preventDefault(); pickSuggestion(s); }}>{s}</button>
      </li>
    {/each}
  </ul>
{/if}
...
<style>
  ...
  form.compose {
    ...
    /* Fresh stacking context so the To autocomplete dropdown can't be
       out-stacked by anything painted by the surrounding page chrome. */
    isolation: isolate;
  }
  ...
  .row {
    ...
    /* Every row gets an explicit, low stacking layer so the To row
       (z-index: 50) is unambiguously above its Cc/Bcc/Subject siblings.
       Previously these rows used z-index: auto, which let some browsers
       paint them on top of the absolutely-positioned dropdown. */
    position: relative;
    z-index: 1;
  }
  .row label {
