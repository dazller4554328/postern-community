...
With Postern reachable over Tailscale, harden the host so nothing is
exposed to the public internet: set up key-only SSH, move it onto
Tailscale, and close every inbound port with a firewall. Step-by-step
(with lockout safeguards) in **[Lock the server down](hardening.md)**.
...
| **Back up the vault** | *Settings → Backups.* Encrypted DB ships to S3 / B2 / SFTP / Google Drive on the schedule you pick. Google Drive needs a one-time OAuth setup — see [Google Drive backups](../guides/google-drive-backup.md). |
| **Lock the server down** | Close all public ports, key-only SSH over Tailscale — see [Lock the server down](hardening.md). |
